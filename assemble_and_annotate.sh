...
read1=$2                  #the read 1 filename (the read 2 filename will be constructed from this)
assembly_directory=$3     #path to a folder in which all the genome assemblies will be stored
name_prefix=$4            #prefix to give all genome assemblies (ideally a short and unique genome name)

#set names for classifying file names from genome assemblies
lane=$(sed 's/.*S.*_L00\(.*\)_R.*/\1/' <(echo "$read1"))
echo "$lane"
strain=$(sed 's/.*S\(.*\)_L00.*/\1/' <(echo "$read1"))
echo "$strain"
#echo $read1 to declare $read2
echo "$read1"
read2=$(sed 's/R1/R2/g' <(echo "$read1"))
echo "$read2"
#now create genome names for processing genome assemblies
genome_name=$(echo "$name_prefix"S"$strain"_L"$lane")

#go to $assembly_directory
cd $assembly_directory
#for running SPAdes programme, identify the directory of the SPAdes programme
...
#now to get contig files, go to the genome_name folders
cd "$assembly_directory""/""$genome_name"
#remove unncessary files, and keep only contig fasta files
for file in $(ls | grep -v contigs.fasta)
do
  rm $file
  rm -r $file
done
#now rename the contig file names
mv contigs.fasta "$genome_name"_contigs.fasta

#now we do indexing using BWA programmes and fasta files
...
#mv *fasta.sa INDEX
#mv *fasta.amb INDEX
#declare a variable as a reference to later do mapping using BWA programme
#index="$genome_name"_contigs.fasta
#now do mapping and create bam files
bwa mem -t 10 *fasta "$raw_data_directory""/""$read1" "$raw_data_directory""/""$read2" | samtools view -q 30 -b -@ 10 - | samtools sort -@ 10 - > "$genome_name"_contigs.bam

#then run Pilon programme to detect variants of reads and correct indels
samtools index "$genome_name"_contigs.bam
java -Xmx16G -jar /ebio/abt6_projects7/bacterial_strain_analysis/code/pilon-1.20.jar --genome "$genome_name"_contigs.fasta --frags "$genome_name"_contigs.bam
#then rename contig headnames because they are short and compatible with Pan Genome Analysis
python /ebio/abt6_projects7/bacterial_strain_analysis/code/rename_fasta.py pilon.fasta
...
#renome all pilon fasta files
rm pilon.fasta

#now annotate the pilon-corrected fasta files using Prokka programme
mkdir Annotations
#prokka=/ebio/abt6_projects9/microbiome_analysis/data/software/prokka/bin/prokka
...
cp ./Annotations/PROKKA_*.gbk ./Annotations/"$genome_name"_PROKKA.gbk
